...
class HybridDecoder():
    ...
    def decode(self, inputFilePath, Umax, gamma, gamma_0, dynamicRange, nCols, nRows=1, nBands=1, decodingOrder='bsq'):
                ...
                for row in range(self.nRows-1, -1, -1):
                    for col in range(self.nCols-1, -1, -1):
                        for band in range(self.nBands-1, -1, -1):
                            ...
                            mappedResidual = self.decodeTargetMappedResidual(row, col, band)
                            MappedResidualsMatrix[row, col, band] = mappedResidual
        # Modfied: Reduce the cube dimension to a single vector and convert it to integers
        MappedResidualsMatrix = MappedResidualsMatrix.squeeze().astype('uint16')
