...
class HybridCoder():
    ...
    def __init__(self):
        ...
        self.Sigma = None
        #   High-Resoulutin accumulator list (one Σ per band)
        self.SigmaList = [] 
        #   User define initial value for Sigma for each spectral band (may be a single integer or a list with Nz elements)
        self.Sigma_0 = None      
        #   List containing the user define initial value for Sigma for each spectral band
        self.Sigma_0_List = None
    ...
    def initializeCodingVariables(self):
        ...
        # High-Resoulutin accumulator list (one Σ per band)
        # For each band: 0 <= Σz(0) <= 2^(D+ɣ(0))
        self.SigmaList = []  # Initial value for each band (Values updated in each iteration)
        if type(self.Sigma_0) == int:
            Sigma_0_clipped = self.clip(self.Sigma_0, 0, 2**(self.dynamicRangeInBits+self.gamma_0)-1 )  
            for band in range(0, self.nBands):
                self.SigmaList.append(Sigma_0_clipped)
        elif type(self.Sigma_0) == list:
            for band in range(0, self.nBands):
                Sigma_0_clipped = self.clip(self.Sigma_0[band], 0, 2**(self.dynamicRangeInBits+self.gamma_0)-1 )  
                self.SigmaList.append(Sigma_0_clipped)
        # Active prefix
        self.ActivePrefix = []
    ...
    def codeImageTail(self):
        ...
        for i in range(0, 16):    
            fulshOutputCodeWord = self.Table_FlushInputToOutputCodeWords[i].get(self.ActivePrefix[i])
            self.writeBinaryStringToBitStream(fulshOutputCodeWord)  
            flushValues.append(fulshOutputCodeWord)      
    ...
    def codeTargetMappedResidual(self, MappedResidualsMatrix, row, col, band):
        # Get the corresponding mapped residual
        mappedResidual = MappedResidualsMatrix[row, col, band]
        # Get the high resolution accumulator for the target band
        self.getHighResolutionAccumulatorForTargetBand(band)
        # Code first mapped residual of each band as a plain binary with D bits
        if row == 0 and col == 0:
        ...
        # Store the high resolution accumulator value in the list for encoding it in the image tail
    ...
    def setConfigurationParameters(self, Umax=16, gamma=8, gamma_0=4, Sigma_0=32768):
        #   Unary Length Limit: 8 <= Umax <= 32
    ...
    def code(self, dynamicRangeInBits, headerBinaryString, outputFilePath, MappedResidualsMatrix, codingOrder='bil', outputWordSize=1):

        # Set the input varialbes
        #   Dynamic range (D)
        self.dynamicRangeInBits = dynamicRangeInBits
        #   Output word size
        self.outputWordSize = outputWordSize

        # Increase the number of dimensions of the MappedResidualsMatrix to 3 if necessary
        while MappedResidualsMatrix.ndim < 3: 
            MappedResidualsMatrix = numpy.expand_dims(MappedResidualsMatrix, axis=MappedResidualsMatrix.ndim)

        # Get the array shape
        [self.nRows, self.nCols, self.nBands] = MappedResidualsMatrix.shape
        ...
        # Code each mapped residual in the corresponding order (BSQ | BIL | BIP)
        #   BSQ
        if codingOrder == 'bsq':
            for band  in range(0, self.nBands):
                for row in range(0, self.nRows):
                    for col in range(0, self.nCols):
                        ...
                        self.codeTargetMappedResidual(MappedResidualsMatrix, row, col, band)
        #   BIL
        if codingOrder == 'bil':
            for row in range(0, self.nRows):
                for band  in range(0, self.nBands):
        ...
        inputBits = self.nRows * self.nCols * self.nBands * self.dynamicRangeInBits
