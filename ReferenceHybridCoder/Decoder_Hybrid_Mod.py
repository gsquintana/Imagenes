...
class HybridDecoder():
    ...
    def initializeDecodingVariables(self):        
        ...
        print(self.GammaList[len(self.GammaList) - 1])
        # Bitstream variables 
    ...
    def UpdateHighResolutionAccumulatorAndCounter(self, mappedResidual, index):
        ...
        rescaling = self.RescalingList[index]
        self.Gamma = self.GammaList[index]
        # If Γ(t) was rescaled
        if rescaling:
    ...
    def decodeTargetMappedResidual(self, index):
        if index == 0:
            ...
            self.UpdateHighResolutionAccumulatorAndCounter(mappedResidual, index)
    ...
    def decode(self, inputFilePath, Umax, gamma, gamma_0, dynamicRange, inputLength):
        ...
        MappedResidualsMatrix = numpy.zeros(self.inputLength)
        # Decode each mapped residual one by one in the corresponding order (BSQ | BIL | BIP)
        for index in range(0, self.inputLength):
            # Store the mapped residual value
            mappedResidual = self.decodeTargetMappedResidual(index)
            MappedResidualsMatrix[index] = mappedResidual
    ...
    def generateMappedResidualsLogFile(self, MappedResidualsMatrix, filePath='MappedResiduals.log'):
        mappedResidualsFiles = filePath
        filePointer = open(mappedResidualsFiles, 'wb')
        for index in range(0, self.inputLength):
            filePointer.write(MappedResidualsMatrix[index].astype(numpy.uint16).tobytes())
        filePointer.close()
